...
gpi_ObjectUpdater::gpi_ObjectUpdater(QObject *parent)
    : QObject{parent}
{
gpi_update();
}

void gpi_ObjectUpdater::gpi_update()
{
    qDebug() << "on_gpi_pushButton_update_clicked()";
    //ui->statusbar->showMessage("Происходит запрос на сервер");
    //QMessageBox::information(this, "Процесс обновления", "Происходит запрос на сервер");
    networkManager = new QNetworkAccessManager();

    // Подключаем networkManager к обработчику ответа
    connect(networkManager, &QNetworkAccessManager::finished, this, &gpi_ObjectUpdater::onResult);
...
void gpi_ObjectUpdater::onResult(QNetworkReply *reply)
{
